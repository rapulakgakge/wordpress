...
class StyleProcessor
{
	...
	static function ImgLazyBgGetStyleBlocks( $aSel, $lazyBg, $important = false )
	{
		...
		$rule -> setValue( $lazyBg -> info ? new Sabberworm\CSS\Value\URL( new Sabberworm\CSS\Value\CSSString( LazyLoad_SrcSubst( $lazyBg -> info, true ) ) ) : 'none' );
		$rule -> setIsImportant( $important || $lazyBg -> isImportant );

		$block = new Sabberworm\CSS\RuleSet\DeclarationBlock();
		$block -> setSelectors( StyleProcessor::AppendSelectors( $aSel, array( '.lzl', '.lzl-ing' ) ) );
		$block -> addRule( $rule );
	...
	static function ImgSzAlternativesGetStyleBlocks( $aSel, $imgSzAlternatives, $important = false, $bLazyBg = false )
	{
		...
		foreach( $imgSzAlternatives -> a as $dim => $imgSzAlternative )
		{
			...
			$rule = new Sabberworm\CSS\Rule\Rule( 'background-image' );
			$rule -> setValue( $imgSzAlternative[ 'img' ] !== null ? new Sabberworm\CSS\Value\URL( new Sabberworm\CSS\Value\CSSString( $imgSzAlternative[ 'img' ] ) ) : 'none' );
			$rule -> setIsImportant( $important || $imgSzAlternatives -> isImportant );
			...
			if( $bLazyBg && $imgSzAlternative[ 'img' ] !== null )
			{
				...
				$ruleAdd -> setValue( $imgSzAlternative[ 'img' ] );
	...
	static function AdjustItemAdaptImg( &$ctxProcess, $settImg, $doc, $item, $itemCssClass = null )
	{
		...
		$item -> setAttribute( 'data-ai-bg', '-' . implode( '-', array( 			1920,	1366,	992,	768,	480,	360	 ) ) . '-0-' );
