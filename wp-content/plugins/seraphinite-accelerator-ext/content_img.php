...
{
	$type = Gen::GetFileName( $imgSrc -> mimeType );
...
function Images_ProcessSrcSizeAlternatives( $imgSzAlternatives, &$ctxProcess, $imgSrc, $settCache, $settImg, $settCdn, $isImportant = false )
{
	...
	static $g_aAdaptDimsScale = array( 	1.01,	1.01,	1.03,	1.05,	1.0,	1.0	 );

	foreach( array( 			1920,	1366,	992,	768,	480,	360	 ) as $i => $cx )
	{
		$scale = ( float )$info[ 'cx' ] / $cx;
		if( $scale < $g_aAdaptDimsScale[ $i ] || !( Gen::GetArrField( $settImg, array( 'szAdaptBgCxMin' ), 0 ) <= $cx ) )
			continue;
		...
		$imgSzAlternatives -> a[ ( string )$cx ] = array( 'img' => $imgSrcAlter, 'sz' => array( $cx, $cy ) );
	}
